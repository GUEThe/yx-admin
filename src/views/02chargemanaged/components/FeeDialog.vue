...
@Component({})
export default class FeeDialog extends Vue {
  ...
  async onshowDialogChangeAsync(val: boolean, old: boolean) {
    if (this.type) {
      ...
      const { data } = await api.GetFeeItem({ id: this.id });
      this.formData = data!;
    } else {
  ...
  async onSubmitAsync() {
    const { data } = this.type ? await api.PutFee({ id: this.id, model: this.formData }) : await api.PostFee({ model: this.formData });
    if (data) {
